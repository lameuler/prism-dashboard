...
{#if showing === false && LINK}
    <div class="fixed right-0 bottom-0 max-w-full box-border p-4">
...
{:else if showing === true && LINK}
    <div class="fixed right-0 bottom-0 max-w-full box-border p-4 w-80">
        <div transition:fly={{y: 100}} class="rounded-xl bg-gradient-to-b from-slate-800/50 to-black/70 backdrop-blur-md px-4 py-3 border-2 border-purple-600">
            <div class="text-xl font-bold flex items-center gap-2">
                ...
                <button class="p-2 -mr-1 rounded-full text-gray-400 hover:bg-gray-800 hover:text-gray-100" on:click={()=>{ showing = false; localStorage.setItem('popup', 'false') }}>
                    <svg class="h-6 w-6" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
